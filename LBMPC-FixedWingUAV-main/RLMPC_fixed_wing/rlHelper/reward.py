...
class Reward(object):
    ...
    def __init__(self, base_reward_elements: Tuple, shaping_reward_elements: Tuple,
        ...
        self.shaping_weights = np.array(shaping_weights) if shaping_weights else np.ones(
            (len(shaping_reward_elements),))
    ...
    def agent_reward(self) -> float:
        ...
        sum_reward = np.sum(self.base_reward_elements * self.base_weights) + np.sum(
            self.shaping_reward_elements * self.shaping_weights)
    ...
    def assessment_reward(self) -> float:
        ...
        return np.sum(
            self.base_reward_elements * self.base_weights) / self.base_weights_sum if self.base_weights_sum else 0  # it may happen that there is only one zero weight component
...
class NormalisedComponent(RewardComponent, ABC):
    ...
    def __init__(self,
        ...
        self.weight = weight
        try:
            self.state_index_of_value = state_variables.index(
                prop)  # will raise an exception if prop is not part of the state_variables
        except ValueError:
    ...
    def _set_target(self, target: Union[int, float, prp.Property, prp.BoundedProperty],
        ...
        """
        if isinstance(target, float) or isinstance(target,
                                                   int):  # TODO: this distinction may be more elegant using the @singledispatch decorator
...
def normalise_error_quadratic(absolute_error: float,
                              max_error: float) -> float:  # TODO: this is now not a quadratic component any more; pure qudratic calc yields far too high rewards with "low" deltaA values
    """
